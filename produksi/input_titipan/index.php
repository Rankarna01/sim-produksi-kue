...
<body class="text-slate-800 antialiased h-screen flex overflow-hidden bg-background" onclick="closeAllDropdowns(event)">
    ...
    <div class="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <!-- Menambah pb-24 agar tombol paling bawah tidak tertutup oleh UI Safari iPhone -->
        <main class="flex-1 overflow-x-hidden overflow-y-auto p-4 sm:p-6 lg:p-8 pb-24 sm:pb-12">
            ...
            <div class="w-full max-w-5xl mx-auto mt-2 sm:mt-4">
                ...
                <div class="mb-6 bg-surface p-6 rounded-2xl shadow-sm border border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-4 text-center sm:text-left">
                    <div class="flex flex-col sm:flex-row items-center gap-4 sm:gap-6">
                        ...
                        <div>
                            <h2 class="text-2xl sm:text-3xl font-bold text-slate-800 tracking-tight leading-tight">Catat Produk Titipan</h2>
                            <p class="text-sm text-secondary mt-1">Keluarkan barang titipan UMKM untuk dikirim ke Store.</p>
                        </div>
                    </div>
                </div>

                <div class="bg-surface rounded-2xl shadow-sm border border-slate-200 relative overflow-hidden mb-8">
                    ...
                    <form id="formProduksi" class="p-5 sm:p-8 md:p-10">
                        <div class="space-y-6">
                            ...
                            <div class="bg-amber-50 p-4 sm:p-6 rounded-2xl border border-amber-100">
                                ...
                                <select id="employee_id" name="employee_id" required class="w-full px-4 py-3.5 sm:py-4 border-2 border-amber-200 rounded-xl focus:ring-0 focus:border-amber-500 outline-none transition-all bg-white text-base font-bold text-amber-900 cursor-pointer shadow-sm appearance-none">
                            ...
                            <div class="bg-slate-50 p-4 sm:p-6 rounded-2xl border border-slate-100">
                                ...
                                <button type="button" onclick="addProductRow()" class="mt-5 bg-slate-100 hover:bg-slate-200 text-slate-700 px-4 py-4 sm:py-3 rounded-xl text-sm font-bold transition-all flex items-center gap-2 border border-slate-300 border-dashed w-full sm:w-auto justify-center active:scale-95">
                            ...
                            <div class="bg-slate-50 p-4 sm:p-6 rounded-2xl border border-slate-100">
                                ...
                                <select id="warehouse_id" name="warehouse_id" required class="w-full px-4 py-3.5 sm:py-4 border-2 border-slate-200 rounded-xl focus:ring-0 focus:border-amber-500 outline-none transition-all bg-white text-base font-medium cursor-pointer shadow-sm appearance-none">
                            ...
                            <div class="bg-slate-50 p-4 sm:p-6 rounded-2xl border border-slate-100">
                                ...
                                <textarea id="notes" name="notes" rows="2" class="w-full px-4 py-3.5 sm:py-4 border-2 border-slate-200 rounded-xl focus:ring-0 focus:border-amber-500 outline-none transition-all bg-white text-base font-medium shadow-sm placeholder:text-slate-300" placeholder="Ketik catatan di sini..."></textarea>
                            </div>
                            
                        </div>

                        <!-- Tombol Proses - Diberi padding-bottom lebih di iPhone -->
                        <div class="mt-8 sm:mt-10 pt-6 sm:pt-8 border-t border-slate-100">
                            <button type="submit" class="w-full bg-amber-500 hover:bg-amber-600 text-white py-4.5 sm:py-5 rounded-2xl text-lg sm:text-xl font-bold transition-all shadow-lg active:scale-95 flex items-center justify-center gap-3">
                                <i class="fa-solid fa-paper-plane text-xl sm:text-2xl"></i> 
    ...
    <!-- Modal Sukses dengan Fix Z-Index untuk iOS -->
    <div id="modal-sukses" class="fixed inset-0 z-[100] flex items-center justify-center hidden px-4">
        ...
        <div class="relative bg-surface w-full max-w-sm rounded-3xl shadow-2xl z-[110] transform transition-all text-center p-6 sm:p-8">
            ...
            <div class="space-y-3 relative">
                <button id="btnCetak" type="button" class="w-full bg-slate-800 hover:bg-slate-900 text-white py-4 rounded-xl font-bold transition-all flex items-center justify-center gap-2 shadow-md active:scale-95">
                    <i class="fa-solid fa-print text-xl"></i> Cetak Struk
                </button>
                <button onclick="selesaiProduksi()" type="button" class="w-full bg-slate-100 hover:bg-slate-200 text-slate-700 py-3.5 rounded-xl font-bold transition-all active:scale-95">
                    Selesai
                </button>
            </div>
        </div>
