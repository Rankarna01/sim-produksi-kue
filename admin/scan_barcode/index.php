        ...
        <main class="flex-1 overflow-x-hidden overflow-y-auto bg-background p-4 sm:p-6 lg:p-8 flex flex-col items-center justify-start relative">
            ...
            <div class="w-full max-w-xl mt-2 sm:mt-6">
                <div class="text-center mb-6 sm:mb-8">
                    ...
                    <p class="text-xs sm:text-sm text-secondary mt-1 sm:mt-2">Scan struk untuk mengecek dan memvalidasi fisik barang dari dapur.</p>
                </div>
                ...
                <div class="bg-surface p-6 sm:p-10 rounded-3xl shadow-lg border border-slate-200 relative overflow-hidden text-center mb-6">
                    ...
                    <form id="formScan" class="relative">
                        <input type="text" id="barcodeInput" name="barcode" autocomplete="off" 
                            class="w-full text-center text-xl sm:text-3xl font-mono tracking-widest px-4 py-4 sm:py-5 border-2 border-slate-300 rounded-2xl focus:border-primary focus:ring-4 focus:ring-primary/20 outline-none transition-all placeholder:text-slate-300" 
                            placeholder="SCAN DISINI..." required>
        ...
        <div class="relative bg-surface w-full max-w-md rounded-3xl shadow-2xl z-[110] transform transition-all flex flex-col overflow-hidden max-h-[95vh]">
            ...
            <div class="p-6 sm:p-8 flex flex-col h-full">
                <div class="text-center mb-4">
                    <div class="w-16 h-16 bg-primary/10 text-primary rounded-full flex items-center justify-center mx-auto mb-4 text-3xl">
                        <i class="fa-solid fa-boxes-stacked"></i>
                    </div>
                    <h3 class="text-2xl font-bold text-slate-800 mb-1">Cek Fisik Barang!</h3>
                    <p class="text-sm text-secondary">Pastikan seluruh fisik barang di keranjang sesuai dengan daftar ini.</p>
                </div>
                ...
                <div class="bg-slate-50 border border-slate-200 rounded-xl p-4 text-left mb-6 flex-1 flex flex-col overflow-hidden">
                    <div class="flex justify-between items-center mb-3 pb-2 border-b border-slate-200">
                        ...
                        <div class="font-mono font-bold text-primary text-sm" id="konf-invoice">INV-XXXX</div>
                    </div>
                    
                    <div class="flex-1 overflow-y-auto custom-scrollbar pr-2 mb-4 space-y-2 max-h-48" id="konf-list-produk">
                        </div>
                    
                    <div class="grid grid-cols-2 gap-2 text-xs pt-3 border-t border-slate-200">
                        <div class="text-slate-500">Dari Pegawai Dapur: <br><strong class="text-slate-700" id="konf-user">User</strong></div>
                        <div class="text-slate-500">Tujuan Simpan: <br><strong class="text-slate-700" id="konf-gudang">Gudang</strong></div>
                    </div>
                </div>
                ...
                <div class="flex flex-col gap-3 mt-auto shrink-0">
                    <button onclick="prosesValidasi('masuk_gudang')" class="w-full bg-success hover:bg-green-600 text-white py-3.5 rounded-xl text-base font-bold transition-all shadow-md flex items-center justify-center gap-2">
                        <i class="fa-solid fa-check-circle"></i> Ya, Semua Sesuai (Valid)
                    </button>
                    <button onclick="prosesValidasi('ditolak')" class="w-full bg-danger/10 hover:bg-danger hover:text-white text-danger py-3.5 rounded-xl text-base font-bold transition-all flex items-center justify-center gap-2">
                        <i class="fa-solid fa-xmark-circle"></i> Ada yang Kurang/Salah (Tolak)
                    </button>
                    <button onclick="document.getElementById('modal-konfirmasi').classList.add('hidden')" class="w-full bg-slate-100 hover:bg-slate-200 text-slate-600 py-3 rounded-xl text-sm font-semibold transition-all mt-2">
                        Batal Scan
                    </button>
                </div>
            </div>
