...
async function loadData(page = 1) {
    ...
    const url = `logic.php?action=read&page=${currentPage}&status=${status}&filter_date=${currentFilterDate}&start_date=${start_date}&end_date=${end_date}&search=${search}`;
    const res = await fetchAjax(url, 'GET');
    
    if (res.status === 'success') {
        let html = '';
        if (res.data.length === 0) {
            html = '<tr><td colspan="6" class="p-10 text-center text-slate-400 italic">Tidak ada data stok barang.</td></tr>';
        } else {
            res.data.forEach((item) => {
                html += `
                    <tr class="hover:bg-slate-50 transition-colors">
                        <td class="p-5 text-xs font-mono text-slate-600">${item.sku_code}</td>
                        <td class="p-5 text-sm font-bold text-slate-800">${item.material_name}</td>
                        <td class="p-5 text-center text-base font-black text-blue-600">${parseFloat(item.stock)}</td>
                        <td class="p-5 text-xs text-slate-600">${item.unit}</td>
                        <td class="p-5 text-xs text-slate-600">${item.category_name || '-'}</td>
                        <td class="p-5 text-xs font-bold text-slate-700">${item.rack_name || '-'}</td>
                    </tr>
                `;
            });
        }
...
function exportData(type) {
    ...
    url += `?status=${status}&filter_date=${currentFilterDate}&start_date=${start_date}&end_date=${end_date}&search=${search}`;
