...
async function cetakDokumen(po_id, tipe, filePrint) {
    ...
    const confirm = await Swal.fire({
        title: 'Cetak Dokumen?',
        text: `Setelah ${textDesc} dicetak, tombol ini akan TERKUNCI dan butuh izin Manager untuk mencetak ulang.`,
        icon: 'warning',
    ...
    if (confirm.isConfirmed) {
        ...
        if(res.status === 'success') {
            ...
            loadDataPO(); 
            Swal.fire({title: 'Terkunci', text: `${textDesc} berhasil dikunci.`, icon: 'success', toast: true, position: 'top-end', showConfirmButton: false, timer: 1500});
        } else {
...
// ... [BIARKAN FUNGSI LAIN (LoadDraftPO, simpanPO, modal dll) TETAP SAMA SEPERTI ASLINYA] ...
// (Kamu dapat mempertahankan sisa fungsi buat PO dan modal sesuai aslinya)
        ...
        cartPO.forEach((item, index) => {
            html += `
                <tr class="hover:bg-slate-50 transition-colors">
                    ...
                    <td class="p-5 text-center"><span class="font-black text-blue-600 text-sm">${item.qty}</span> <span class="text-[10px] font-black uppercase text-slate-400 ml-1">${item.unit}</span></td>
                    <td class="p-5 text-center"><button type="button" onclick="hapusCart(${index})" class="w-6 h-6 rounded bg-rose-50 hover:bg-rose-500 text-rose-500 hover:text-white transition-all flex items-center justify-center mx-auto shadow-sm"><i class="fa-solid fa-xmark text-[10px]"></i></button></td>
                </tr>
            `;
        });
    }
