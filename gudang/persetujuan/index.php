    ...
    <div class="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <main class="flex-1 overflow-x-hidden overflow-y-auto p-4 sm:p-6 lg:p-8" x-data="{ tab: '<?= $defaultTab ?>' }">
            ...
            <div class="relative w-full mb-6">
                <div class="absolute top-0 left-0 h-full w-8 bg-gradient-to-r from-slate-50 to-transparent pointer-events-none z-10"></div>
                <div class="absolute top-0 right-0 h-full w-8 bg-gradient-to-l from-slate-50 to-transparent pointer-events-none z-10"></div>
                
                <div class="flex flex-nowrap gap-3 bg-white p-2 rounded-2xl shadow-sm border border-slate-200 overflow-x-auto hide-scroll scroll-smooth snap-x">
                    
                    <?php if (hasPermission('persetujuan_po')): ?>
                    <button @click="tab = 'po'; loadPOApproval();" :class="tab === 'po' ? 'bg-blue-600 text-white shadow-md shadow-blue-200' : 'text-slate-500 hover:bg-slate-50'" class="snap-start shrink-0 px-5 sm:px-6 py-3 rounded-xl text-[10px] sm:text-xs font-black uppercase tracking-widest transition-all">
                        <i class="fa-solid fa-cart-shopping mr-2"></i> Purchase Orders
                    </button>
                    <?php endif; ?>

                    <?php if (hasPermission('persetujuan_pr')): ?>
                    <button @click="tab = 'permintaan'; loadPermintaan(1);" :class="tab === 'permintaan' ? 'bg-blue-600 text-white shadow-md shadow-blue-200' : 'text-slate-500 hover:bg-slate-50'" class="snap-start shrink-0 px-5 sm:px-6 py-3 rounded-xl text-[10px] sm:text-xs font-black uppercase tracking-widest transition-all">
                        <i class="fa-solid fa-clipboard-list mr-2"></i> Permintaan Barang (PR)
                    </button>
                    <?php endif; ?>

                    <?php if (hasPermission('persetujuan_masuk_manual')): ?>
                    <button @click="tab = 'manual'; loadManualApproval();" :class="tab === 'manual' ? 'bg-blue-600 text-white shadow-md shadow-blue-200' : 'text-slate-500 hover:bg-slate-50'" class="snap-start shrink-0 px-5 sm:px-6 py-3 rounded-xl text-[10px] sm:text-xs font-black uppercase tracking-widest transition-all">
                        <i class="fa-solid fa-box mr-2"></i> Barang Masuk
                    </button>
                    <?php endif; ?>

                    <?php if (hasPermission('persetujuan_keluar_manual')): ?>
                    <button @click="tab = 'keluar'; loadKeluarApproval();" :class="tab === 'keluar' ? 'bg-rose-600 text-white shadow-md shadow-rose-200' : 'text-slate-500 hover:bg-slate-50'" class="snap-start shrink-0 px-5 sm:px-6 py-3 rounded-xl text-[10px] sm:text-xs font-black uppercase tracking-widest transition-all">
                        <i class="fa-solid fa-box-open mr-2"></i> Barang Keluar
                    </button>
                    <?php endif; ?>

                    <?php if (hasPermission('persetujuan_izin_cetak')): ?>
                    <button @click="tab = 'izin'; loadIzinCetak();" :class="tab === 'izin' ? 'bg-amber-500 text-white shadow-md shadow-amber-200' : 'text-slate-500 hover:bg-slate-50'" class="snap-start shrink-0 px-5 sm:px-6 py-3 rounded-xl text-[10px] sm:text-xs font-black uppercase tracking-widest transition-all">
            ...
            <div x-show="tab === 'permintaan'" x-cloak x-transition class="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm overflow-hidden flex flex-col">
                <div class="p-6 border-b border-slate-100 flex flex-col sm:flex-row sm:justify-between sm:items-center bg-slate-50/50 gap-4">
                    ...
                    <div class="flex flex-wrap items-center gap-3">
                        <button onclick="bukaModalBuatRequest()" class="flex-1 sm:flex-none bg-primary hover:bg-blue-700 text-white px-4 py-2.5 rounded-xl text-xs font-bold transition-all shadow-sm">
                            <i class="fa-solid fa-plus mr-1"></i> Buat Request Baru
                        </button>
                        <button onclick="loadPermintaan(1)" class="text-blue-600 hover:rotate-180 transition-transform duration-500 bg-blue-50 w-10 h-10 rounded-full flex items-center justify-center shrink-0">
            ...
            <div x-show="tab === 'po'" x-transition class="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm overflow-hidden flex flex-col">
                <div class="p-6 border-b border-slate-100 flex flex-col sm:flex-row sm:justify-between sm:items-center bg-slate-50/50 gap-4">
                    ...
                    <button onclick="loadPOApproval()" class="text-blue-600 hover:rotate-180 transition-transform duration-500 bg-blue-50 w-10 h-10 rounded-full flex items-center justify-center self-end sm:self-auto shrink-0">
                        <i class="fa-solid fa-rotate"></i>
                    </button>
                </div>
                <div class="p-6">
                ...
                <div class="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
                    <div class="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                        <div>
                            ...
                            <p class="text-xs font-medium text-slate-500 mt-1">Data input masuk dari staf yang menunggu validasi agar stok bertambah.</p>
                        </div>
                        <button onclick="loadManualApproval()" class="w-10 h-10 bg-white border border-slate-200 text-slate-500 hover:text-blue-600 rounded-xl shadow-sm flex items-center justify-center transition-colors shrink-0" title="Refresh Data">
                ...
                <div class="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
                    <div class="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                        <div>
                            ...
                            <p class="text-xs font-medium text-slate-500 mt-1">Otorisasi pemotongan stok untuk barang yang rusak atau expired.</p>
                        </div>
                        <button onclick="loadKeluarApproval()" class="w-10 h-10 bg-white border border-slate-200 text-slate-500 hover:text-rose-600 rounded-xl shadow-sm flex items-center justify-center transition-colors shrink-0" title="Refresh Data">
                            <i class="fa-solid fa-rotate-right"></i>
                        </button>
                    </div>
                    <div class="overflow-x-auto custom-scrollbar">
            ...
            <div x-show="tab === 'izin'" x-cloak x-transition class="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
                <div class="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                    <div>
                        ...
                        <p class="text-xs font-medium text-slate-500 mt-1">Buka kembali kunci akses print untuk dokumen PO.</p>
                    </div>
                    <button onclick="loadIzinCetak()" class="w-10 h-10 bg-white border border-slate-200 text-slate-500 hover:text-amber-600 rounded-xl shadow-sm flex items-center justify-center transition-colors shrink-0" title="Refresh Data">
                        <i class="fa-solid fa-rotate-right"></i>
                    </button>
                </div>
                <div class="overflow-x-auto custom-scrollbar">
            ...
            <div x-show="tab === 'histori'" x-cloak x-transition class="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
                ...
                <div class="p-4 border-b border-slate-100 bg-white flex flex-col md:flex-row gap-4 items-center">
                    <select id="histori_modul" onchange="loadHistori()" class="px-4 py-2.5 border border-slate-300 rounded-xl outline-none text-xs font-bold text-slate-600 bg-slate-50 w-full md:w-auto">
                    ...
                    <select id="histori_status" onchange="loadHistori()" class="px-4 py-2.5 border border-slate-300 rounded-xl outline-none text-xs font-bold text-slate-600 bg-slate-50 w-full md:w-auto">
                    ...
                    <div class="relative w-full flex-1">
                        ...
                        <input type="text" id="histori_search" placeholder="Cari No Dokumen..." class="w-full pl-9 pr-3 py-2.5 border border-slate-300 rounded-xl outline-none text-xs font-bold text-slate-600 bg-slate-50" onkeyup="cariHistori()">
                    </div>
                </div>
                <div class="overflow-x-auto custom-scrollbar">
                    <table class="w-full text-left text-sm min-w-[900px]">
    ...
    <div id="modal-detail-po" class="fixed inset-0 z-50 flex items-center justify-center hidden px-4">
        ...
        <div class="relative bg-surface w-full max-w-3xl rounded-3xl shadow-xl z-10 transform transition-all flex flex-col overflow-hidden max-h-[90vh]">
            <div class="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50 rounded-t-3xl">
                ...
                <button onclick="closeModal('modal-detail-po')" class="text-secondary hover:text-danger transition-colors"><i class="fa-solid fa-xmark text-xl"></i></button>
            </div>
    ...
    <div id="modal-buat-req" class="fixed inset-0 z-50 flex items-center justify-center hidden px-4">
        ...
        <div class="relative bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl z-10 transform transition-all flex flex-col overflow-hidden">
            <div class="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                ...
                <button onclick="closeModal('modal-buat-req')" class="text-slate-400 hover:text-red-500 transition-colors"><i class="fa-solid fa-xmark text-xl"></i></button>
            </div>
