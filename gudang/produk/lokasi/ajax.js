...
function resetForm() {
    ...
    document.getElementById('modal-title').innerText = 'Tambah Lokasi Rak';
}
...
async function loadData(page = 1) {
    ...
    if (response.status === 'success') {
        ...
        if (response.data.length === 0) {
            html = '<tr><td colspan="4" class="p-8 text-center text-secondary font-medium">Tidak ada data lokasi rak ditemukan.</td></tr>';
        } else {
            response.data.forEach((item, index) => {
                ...
                html += `
                    <tr class="hover:bg-slate-50 transition-colors">
                        ...
                        <td class="p-4">
                            <span class="bg-indigo-50 text-indigo-700 font-bold px-3 py-1 rounded-lg border border-indigo-100 uppercase tracking-widest">
                                <i class="fa-solid fa-location-dot mr-1 opacity-50"></i> ${item.name}
                            </span>
                        </td>
                        <td class="p-4 text-slate-500 text-sm italic">${item.description || 'Tidak ada keterangan'}</td>
...
document.getElementById('formRak').addEventListener('submit', async function(e) {
    e.preventDefault();
...
function editData(item) {
    ...
    document.getElementById('modal-title').innerText = 'Edit Lokasi Rak';
    openModal('modal-rak');
}
...
async function deleteData(id) {
    const result = await Swal.fire({
        title: 'Hapus Rak?',
        text: "Rak yang sudah ada isi barangnya tidak bisa dihapus.",
        icon: 'warning',
