...
try {
    ...
    // 3. CREATE DATA MANUAL (Dengan Status PENDING, tanpa update stok)
    if ($action === 'save') {
        ...
        if (empty($material_id) || $qty <= 0 || empty($expiry_date)) {
            echo json_encode(['status' => 'error', 'message' => 'Barang, Jumlah, dan Tanggal Kadaluarsa wajib diisi!']); exit;
        }
       ...
       // Insert Transaksi
        $stmt = $pdo->prepare("INSERT INTO barang_masuk (transaction_no, material_id, supplier_id, qty, source, expiry_date, notes, user_id, status) VALUES (?, ?, ?, ?, 'Manual', ?, ?, ?, ?)");
        $stmt->execute([$trx_no, $material_id, $supplier_id, $qty, $expiry_date, $notes, $user_id, $status_awal]);
        ...
        if ($req_approval == 0) {
            ...
            $msg = 'Barang Masuk disimpan dan stok langsung bertambah (Auto-Approve)!';
        } else {
            $msg = 'Pengajuan Barang Masuk berhasil dibuat! Menunggu persetujuan Owner.';
        }
