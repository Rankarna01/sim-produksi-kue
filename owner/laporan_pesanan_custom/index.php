...
// checkPermission('laporan_pesanan_custom'); // Aktifkan jika sistem permission sudah siap
$page_title = "Laporan Pesanan Custom - RotiKu ERP";
?>
<!DOCTYPE html>
<html lang="id">
<head>
    ...
    <style>
        ...
        @media print {
            ...
            th, td { border: 1px solid #e2e8f0 !important; padding: 8px !important; color: #000 !important; font-size: 12px !important; }
    ...
    <div class="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <main class="flex-1 overflow-x-hidden overflow-y-auto p-4 sm:p-6 lg:p-8 w-full">
            ...
            <div id="print-header" class="hidden">
                <h1 style="font-size: 24px; font-weight: bold; margin-bottom: 5px;">LAPORAN PESANAN CUSTOM ITEM</h1>
                <p id="print-periode" style="font-size: 14px; color: #666;">Dicetak pada: <?= date('d/m/Y H:i') ?></p>
            ...
            <div class="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 no-print">
                <div>
                    <h2 class="text-2xl font-bold text-slate-800 tracking-tight">Laporan Pesanan Custom</h2>
                    <p class="text-sm text-secondary mt-1">Pantau riwayat dan status pesanan khusus (Custom Item) dari mesin kasir.</p>
                </div>
            ...
            <div id="form-filter-section" class="bg-surface p-5 rounded-2xl shadow-sm border border-slate-200 mb-6">
                <form id="formFilter" class="flex flex-col md:flex-row gap-4 items-end" onsubmit="event.preventDefault(); loadData();">
                    
                    <div class="flex-1 w-full">
                        <label class="block text-xs font-bold text-slate-500 mb-1 uppercase tracking-wider">Cari Pesanan / Pelanggan</label>
                        <input type="text" id="filter_search" placeholder="Misal: Kue Ultah, Budi..." class="w-full px-4 py-2 border border-slate-300 rounded-xl focus:border-primary outline-none text-sm bg-white">
                    </div>

                    <div class="flex-1 w-full">
                        <label class="block text-xs font-bold text-slate-500 mb-1 uppercase tracking-wider">Status Dapur</label>
                        <select id="filter_status" class="w-full px-4 py-2 border border-slate-300 rounded-xl focus:border-primary outline-none text-sm bg-white cursor-pointer">
            ...
            <div class="bg-surface rounded-2xl shadow-sm border border-slate-200 overflow-hidden print:shadow-none print:border-none flex flex-col">
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse min-w-[900px]" id="table-laporan-excel">
                        <thead>
                            <tr class="bg-slate-50 border-b border-slate-200 text-xs text-secondary uppercase tracking-wider print:bg-slate-100">
                                ...
                                <th class="p-4 font-bold">Nama Pesanan Custom</th>
                        ...
                        <tbody id="table-body" class="text-sm divide-y divide-slate-100">
                            <tr><td colspan="9" class="p-8 text-center text-secondary"><i class="fa-solid fa-circle-notch fa-spin mr-2"></i>Memuat data...</td></tr>
                        </tbody>
                    </table>
                </div>
            </div>
